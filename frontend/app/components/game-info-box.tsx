...
import { MAX_GAME_INFO_BOX_SAVED_ITEMS, DEFAULT_GAME_INFO_BOX_DISPLAY_ITEMS } from "../utils/gameConfig";
...
export function GameInfoBox({ info, storageKey, title }: GameInfoBoxProps): ReactNode {
    const styleByTitle: GameInfoBoxStyle = GAME_INFO_BOX_STYLES[title] ?? {
        sectionClass: "rounded-lg border border-secondary p-4",
        titleClass: "text-sm font-semibold",
        textAreaRows: DEFAULT_GAME_INFO_BOX_DISPLAY_ITEMS,
    };
    const { sectionClass, titleClass, textAreaRows }: GameInfoBoxStyle = styleByTitle;
    ...
    useEffect((): void => {
        ...
        setEntries((previousEntries: string[]): string[] => {
            return appendCappedHistoryEntry(previousEntries, trimmedInfo, MAX_GAME_INFO_BOX_SAVED_ITEMS);
        });
    }, [info]);
